...
var QuranTransportV1 = (function(){
  ...
  function normOutboxEntry(key, v, errors){
    ...
    if (!validOrder(v.revelationOrder)) { errors.push('bad_revelation_order'); return null; }
    var requestedAt = isoOrNull(v.requestedAt);
    if (!requestedAt) { errors.push('bad_requested_at'); return null; }
    return {
      requestId: key, surahId: v.surahId, revelationOrder: v.revelationOrder,
      mushafOrder: validOrder(v.mushafOrder) ? v.mushafOrder : null,
