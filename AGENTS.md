...
- **İlham & İbadet Premium (IIP)** — [`ilham-ibadet-premium-plan/`](ilham-ibadet-premium-plan/README.md) — 24 kartlık hub yenileme programı (A: görsel/kabul, B: mimari/okuyucu/vakit, C: kalıcılık/offline/teslim). Durum kaynağı [`IIP-STATE.json`](ilham-ibadet-premium-plan/IIP-STATE.json); kart kataloğu [`06-UYGULAMA-KARTLARI.md`](ilham-ibadet-premium-plan/06-UYGULAMA-KARTLARI.md); kanıt [`evidence/`](ilham-ibadet-premium-plan/evidence/); takip [`tracking/`](ilham-ibadet-premium-plan/tracking/). **Bilinen durum (2026-09-22 denetimi):** state 24/24 done beyan ediyor; `plan-check.mjs` (+`--self-test`) ve `plan-check.integration.py` artık **exit 0**, ancak `deviceAcceptance=not_verified` (cihaz teyidi yok) ve p50/p95 ölçülmedi. Ölçüm tablosu + sıralı kapatma adımları: [`DUZELTME-PROMPTLARI.md`](ilham-ibadet-premium-plan/DUZELTME-PROMPTLARI.md) (P00→P17) ve [`evidence/DENETIM-BASELINE.md`](ilham-ibadet-premium-plan/evidence/DENETIM-BASELINE.md). Testler: `tests/app/test_iip_*.js` (16 fixture). **KARDEŞ-KART REGRESYON KAPISI (P13):** `tests/FIXTURE-MAP.json` her fixture'ın hangi üretim dosyasına bağlı olduğunu kaydeder ve `plan-check.mjs` bunu zorlar — bir kart paylaşılan bir üretim dosyasına yazdığında o dosyanın **koruyucu fixture'larını** `fixtureFiles` listesinde beyan etmek ZORUNDADIR. Harita bayatlarsa `node tools/fixture-map-build.mjs --write` ile yenile. (IIP-20/21 `app/core/saygi.js`'e yazıp 13 kardeş fixture'ı sessizce kırdı; kanıt: 30 fixture saygi.js'e bağlı, kartlar 3'ünü beyan etmişti.) **PERFORMANS:** yerel VM ölçümü `node tests/app/test_iip_perf_measure.js` (p50 0.43-0.62 ms, p95 1.14-1.48 ms); hedef **cihaz** p95 ≤200 ms HÂLÂ ÖLÇÜLMEDİ — protokol [`evidence/IIP-PERF-PROTOKOL.md`](ilham-ibadet-premium-plan/evidence/IIP-PERF-PROTOKOL.md). VM sayısı cihaz PASS'ı DEĞİLDİR.
