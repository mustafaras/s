...
- For any reminder release or live action, read [`docs/reminders/APP-REMINDER-APPROVAL-GATE.md`](docs/reminders/APP-REMINDER-APPROVAL-GATE.md). The standing user delivery policy is `after_each_prompt`: after a prompt closes with all gates PASS, deliver only the current `main` fast-forward to GitHub Pages and record remote/CI/live evidence. `releaseApproval` remains `NOT_APPROVED` outside that bounded prompt-delivery scope; tag, force-push, other remotes, arbitrary external writes and `mustafaras/seyma-data` still require separate approval.
...
```bash
node tests/test_faz10_sync.js   # sync.js conflict-merge harness (mocked fetch)
node tests/test_faz11_panel.js  # panel.html helper/render harness
node tests/test_panel_p0_sync.js # PANEL-01 receipt/revision + anti-clobber fixture
node tests/test_panel_p1_projection.js # PANEL-02 coverage/redaction/projection fixture
node tests/test_panel_p3_root_modules.js # PANEL-03 root modules/privacy/mismatch fixture
node tests/test_panel_p4_provenance.js # PANEL-04 therapy/notification/provenance fixture
