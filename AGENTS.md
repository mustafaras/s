...
- **İlham & İbadet Premium (IIP)** — [`archive/ilham-ibadet-premium-plan/`](archive/ilham-ibadet-premium-plan/README.md) — 24 kartlık hub yenileme programı. **🏁 KAPANDI (2026-09-22): 24/24 kart + P00–P17 düzeltmeleri tamamlandı, kalan iş YOK.** Kapanış: [`docs/IIP-KAPANIS.md`](docs/IIP-KAPANIS.md); durum [`IIP-STATE.json`](archive/ilham-ibadet-premium-plan/IIP-STATE.json) (`nextExecutableCard: null`). Ölçüm: **156 fixture PASS / 0 FAIL**,
`plan-check.mjs` + `--self-test` + `.integration.py` exit 0, `evidence-reconcile.mjs`
16 makbuz/166 komut/0 drift. **Dürüstçe açık:** hedef cihaz p50/p95 sayısı yok
(`user_accepted`), cihaz kabulü kullanıcı beyanı (`user_attested`, `agentVerified:false`),
gerçek ekran okuyucu testi ajan yapmadı. **KARDEŞ-KART REGRESYON KAPISI:** `tests/FIXTURE-MAP.json`
+ `plan-check.mjs` — paylaşılan üretim dosyasına yazan kart o dosyanın koruyucu fixture'larını
`fixtureFiles`'ta beyan etmek ZORUNDA; harita bayatlarsa `node tools/fixture-map-build.mjs --write`.
**Klasör 2026-09-23'te `archive/` altına TAŞINDI** (kullanıcı kararı; taşımanın kırdığı
4 nokta — `plan-check` kart/link çözümü, `evidence-reconcile` yol sabiti,
`integration.py` kök çözümü, `test_iip_17.js` yazma yolu — arşiv-farkındalıklı hâle
getirildi; taşıma sonrası 6 kapı exit 0, 156 fixture PASS/0 FAIL). Yeni bir IIP işi
eyrı kapsam onayı ister.
- **Kur'an Arapçası Öğreniyorum (KAO)** — [`kuran-ogreniyorum/`](kuran-ogreniyorum/README.md) — 30 kartlık Kur'an Arapçası öğrenme programı (`app/core/quranLearn.js`, `window.SeymaQuranLearn`). **🏁 KAPANDI (2026-09-26)**: kapanış [`KAO-KAPANIS.md`](kuran-ogreniyorum/deliverables/KAO-KAPANIS.md), regresyon [`KAO-REGRESYON.md`](kuran-ogreniyorum/deliverables/KAO-REGRESYON.md), durum `kuran-ogreniyorum/KAO-STATE.json` (`status=completed`), dalga denetimleri `evidence/KAO-D1…D6/AUDIT.md`. Doğrulama: `for f in tests/kao/*.js; do node $f; done`, `node kuran-ogreniyorum/tools/kao-plan-check.mjs`, `node kuran-ogreniyorum/tools/kao-verify-contrast.mjs`. **Açık kararlar:** içerik gzip 159,9 KB > 130 KB bütçe; cihaz kabulü (K3) kullanıcıda. **Tuzaklar:** yeni `App.kao*` handler'ı fx2/v3/surface pinlerini kaydırır; KAO runtime'ı, 4 içerik modülü ve `app/kao.css` tek yayın pinini paylaşır (`index.html`, `sw.js`, `tests/app/test_iip_22.js`); Arapça içerik ve okunuşlar yalnız `tools/kao-lexicon-build.mjs` / `tools/kao-content-freeze.mjs` çıktısından gelir, elle yazılmaz. Yeni KAO işi ayrı kapsam onayı ister.
- **v3.0 tanıtım + kutlama sayfası** (`v3-tanitim/`, sürüm tanıtımı): ayrı bir yüzeydir, `app.js`'e hiç dokunmaz. Açılışta tanıtım görülmediyse `index.html`'in `<head>`'indeki tek inline bootstrap `location.replace` ile yönlendirir ve `app.js` hiç *çalışmaz* (tarayıcı `<body>` betiklerini ön-indirebilir; indirmek çalıştırmak değildir). **Kalıcılık:** `seyma-v3-welcome-v1` (uygulamanın `seyma-reset-v1` anahtarından ayrı namespace; "Verileri sıfırla" silmez). **Sonsuz döngü koruması:** `v3.js` işareti yazdığını geri okuyarak doğrular; depo yazılamıyorsa `?v3done=1` ile döner ve bootstrap bir kez atlar. **Uygulama içi sürüm:** Ayarlar → Hakkında **v3.0** + "3.0'da neler değişti?" köprüsü (düz `<a>`) ve başlangıç ekranı rozeti v3.0. **Gün sayısı SABİT DEĞİL:** `dayCount = diffDays(data.startDate, bugün) + 1` ile türetilir (gerçek veri: `startDate=2026-06-24` → **84**); statik HTML yalnız geçerli bir *varsayılan* taşır, JS gerçek sayıya düzeltir. **Statik anlık görüntü (2026-09-15, kullanıcı isteği):** `v3-tanitim/v3-snapshot.js` — `node tools/v3-snapshot-build.mjs` üretir (salt-okur GET, sayfanın kendi modülleriyle özet, **yalnız sayısal**, yasaklı alan taraması); **tazelik kuralı:** uzak / cihaz deposu / anlık görüntü adaylarından **en taze olan kazanır** (son gün, eşitse kayıt sayısı) — telefonda uygulamanın kendi verisi kazanır ve istatistikler zamanla güncellenir, bayat cihaz kaydı anlık görüntüyü ezemez, ağa yalnız iyileştirebilecekse çıkılır. Ayarlar'da dinamik **"N. gün"** düğmesi (`settings.js`, `dayIndexFor`, düz `<a>`) sayfayı yeniden açar; tekrar açılışta düğme "Uygulamaya dön" der. Repo public olduğu için push kararı kullanıcınındır. **Yedek uzak kaynak:** `v3-source.js` uygulamanın zaten sakladığı `settings.ghToken`/`ghRepo` varsa `data/latest.json`'ı **yalnız-GET** çeker (**repo esastır**; cihaz kaydı yalnız kimlik yokken ya da ağ hatasında yedektir — cihaz kaydı bayat olabilir) (1 MB üstü için `git/blobs` yedeği, UTF-8 `TextDecoder`); **yazmaz**, diske kaydetmez, token'ı yalnız `Authorization` başlığında tutar; kimlik yoksa ağa hiç çıkmaz. Statik HTML varsayılanı gerçek `startDate`'e sabittir (24 Haziran 2026 / 84). **Tuzak:** fx2 fixture'ları App yüzeyini (718) ve tıklama sayısını (391) **düz metin** taramasıyla sabitler; `settings.js`/`render.js` bu taramaya dâhildir ve tarama **yorumları da sayar** — yorumda `App.<ad>=` veya tıklama niteliği adı yazmak pini kaydırır (bir kez yaşandı). **YAYINDA:** 2026-09-15'te `premium-fx-gorsel-yuzey` → `main` fast-forward (269 commit; FX2/SKY/PREM, MON/MON2, AD-52 ve v3.0 sayfası dâhil), tag `v3.0`, Pages run 35006913049 success — bu dosyadaki daha eski "LOCAL-ONLY / push yok" notları o tarihe kadar geçerliydi; canlı: `https://mustafaras.github.io/s/`. Bu sayfayı değiştirmeden önce [`docs/v3-tanitim/README.md`](docs/v3-tanitim/README.md) ve devir/denetim belgesi [`docs/v3-tanitim/DEVIR-PROMPTU.md`](docs/v3-tanitim/DEVIR-PROMPTU.md) oku; doğrulama `node tests/app/test_v3_welcome.js` (287 kontrol) + DEVIR-PROMPTU §6 köprü senaryosu.
...
app/core/quranLearn.js  KAO registry (`window.SeymaQuranLearn`): FSRS, kuyruk ve
                 E1–E11 ekranları (oturum, üniteler, kelime, okuyucu, Seviye 0,
                 ayarlar, telaffuz stüdyosu, günün âyeti, ısı haritası, namaz,
                 istatistik) + panel özeti; app.js yalnız 1-satır App.kao* shim'i tutar.
...
tests/kao/       KAO fixture ailesi (14 dosya); plan ve kanıt: kuran-ogreniyorum/.
