...
- **İlham & İbadet Premium (IIP)** — [`ilham-ibadet-premium-plan/`](ilham-ibadet-premium-plan/README.md) — 24 kartlık hub yenileme programı. **🏁 KAPANDI (2026-09-22): 24/24 kart + P00–P17 düzeltmeleri tamamlandı, kalan iş YOK.** Kapanış: [`docs/IIP-KAPANIS.md`](docs/IIP-KAPANIS.md); durum [`IIP-STATE.json`](ilham-ibadet-premium-plan/IIP-STATE.json) (`nextExecutableCard: null`). Ölçüm: **154 fixture PASS / 0 FAIL**, `plan-check.mjs` + `--self-test` + `.integration.py` exit 0, `evidence-reconcile.mjs` 16 makbuz/166 komut/0 drift. **Dürüstçe açık:** hedef cihaz p50/p95 sayısı yok (`user_accepted`), cihaz kabulü kullanıcı beyanı (`user_attested`, `agentVerified:false`), gerçek ekran okuyucu testi ajan yapmadı. **KARDEŞ-KART REGRESYON KAPISI:** `tests/FIXTURE-MAP.json` + `plan-check.mjs` — paylaşılan üretim dosyasına yazan kart o dosyanın koruyucu fixture'larını `fixtureFiles`'ta beyan etmek ZORUNDA; harita bayatlarsa `node tools/fixture-map-build.mjs --write`. **Klasör taşınmadı** (taşıma 16 makbuz yolunu + `test_iip_17.js` yazma yolunu kırar). Yeni bir IIP işi ayrı kapsam onayı ister.
