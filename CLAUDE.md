...
- **İlham & İbadet Premium (IIP)** — [`archive/ilham-ibadet-premium-plan/`](archive/ilham-ibadet-premium-plan/README.md) — 24 kartlık İlham & İbadet hub yenileme programı. **🏁 KAPANDI (2026-09-22): kalan iş YOK.** Kapanış belgesi [`docs/IIP-KAPANIS.md`](docs/IIP-KAPANIS.md); durum [`IIP-STATE.json`](archive/ilham-ibadet-premium-plan/IIP-STATE.json). Ölçüm: **156 fixture PASS / 0 FAIL**, plan araçları exit 0, makbuz uzlaştırması 0 drift. **Dürüstçe açık:** hedef cihaz p50/p95 sayısı yok; cihaz kabulü kullanıcı beyanı. **KARDEŞ-KART REGRESYON KAPISI:** `tests/FIXTURE-MAP.json` + `plan-check.mjs` — paylaşılan üretim dosyasına yazan kart o dosyanın **koruyucu fixture'larını** `fixtureFiles`'ta beyan etmek zorundadır; harita `node tools/fixture-map-build.mjs --write` ile yenilenir. **Klasör 2026-09-23'te `archive/` altına TAŞINDI** — taşımanın kırdığı 4 nokta (kart/link çözümü, `evidence-reconcile` yol sabiti, `integration.py` kökü, `test_iip_17.js` yazma yolu) arşiv-farkındalıklı hâle getirildi; taşıma sonrası 6 kapı exit 0 (gerekçe/ölçüm: kapanış belgesi §9-§10). Yeni bir IIP işi ayrı kapsam onayı ister.
