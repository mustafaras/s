...
- **İlham & İbadet Premium (IIP)** — [`ilham-ibadet-premium-plan/`](ilham-ibadet-premium-plan/README.md) — 24 kartlık İlham & İbadet hub yenileme programı. Durum kaynağı [`IIP-STATE.json`](ilham-ibadet-premium-plan/IIP-STATE.json); kanıt [`evidence/`](ilham-ibadet-premium-plan/evidence/); takip [`tracking/`](ilham-ibadet-premium-plan/tracking/); kart kataloğu [`06-UYGULAMA-KARTLARI.md`](ilham-ibadet-premium-plan/06-UYGULAMA-KARTLARI.md). **Durum (2026-09-22 denetimi):** state 24/24 done beyan ediyor; plan araçları (`plan-check.mjs`, `--self-test`, `.integration.py`) yeşil, ancak `deviceAcceptance=not_verified` ve p50/p95 ölçülmedi. Denetim tablosu [`evidence/DENETIM-BASELINE.md`](ilham-ibadet-premium-plan/evidence/DENETIM-BASELINE.md); sıralı kapatma adımları [`DUZELTME-PROMPTLARI.md`](ilham-ibadet-premium-plan/DUZELTME-PROMPTLARI.md) (P00→P17). Fixture'lar `tests/app/test_iip_*.js` (16 dosya). **Kural:** bir kart `app/core/saygi.js` gibi paylaşılan bir üretim dosyasına yazdığında, o dosyaya bağlı ÖNCEKİ kartların fixture'larını aynı commit'te yeniden koş — IIP-20/21 bunu atladığı için IIP-05/06/09/12/13 gate'leri sessizce bozuldu.
