...
- **İlham & İbadet Premium (IIP)** — [`ilham-ibadet-premium-plan/`](ilham-ibadet-premium-plan/README.md) — 24 kartlık İlham & İbadet hub yenileme programı. **🏁 KAPANDI (2026-09-22): kalan iş YOK.** Kapanış belgesi [`docs/IIP-KAPANIS.md`](docs/IIP-KAPANIS.md); durum [`IIP-STATE.json`](ilham-ibadet-premium-plan/IIP-STATE.json). Ölçüm: 154 fixture PASS / 0 FAIL, plan araçları exit 0, makbuz uzlaştırması 0 drift. **Dürüstçe açık:** hedef cihaz p50/p95 sayısı yok; cihaz kabulü kullanıcı beyanı. **KARDEŞ-KART REGRESYON KAPISI:** `tests/FIXTURE-MAP.json` + `plan-check.mjs` — paylaşılan üretim dosyasına yazan kart o dosyanın **koruyucu fixture'larını** `fixtureFiles`'ta beyan etmek zorundadır; harita `node tools/fixture-map-build.mjs --write` ile yenilenir. **Klasör taşınmadı** — taşıma 16 kanıt makbuzunun komut yolunu ve `test_iip_17.js`'in yazma yolunu kırar (gerekçe: kapanış belgesi §7). Yeni bir IIP işi ayrı kapsam onayı ister.
