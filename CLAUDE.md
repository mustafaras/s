...
- For the **header live weather/time scene and the remaining premium cosmetics**: the full read-only audit is [`premium-fx-plan/deliverables/TAM-DENETIM-20260908.md`](premium-fx-plan/deliverables/TAM-DENETIM-20260908.md) — 13 findings (4 🔴 · 5 🟡 · 4 🔵), all fixed in commit `1b61c04`, with the fix round recorded in its §E appendix (measured before/after pixel diffs, and four measurement errors of the auditing agent itself). The **next open work** is specified in [`docs/DEVIR-HEADER-HAVA-VE-PREMIUM.md`](docs/DEVIR-HEADER-HAVA-VE-PREMIUM.md): GÖREV A rewrites the header weather effects (currently CSS gradient stripes/dots that do not match Apple Weather's design language — a canvas particle engine in a new `app/core/skyFx.js` is the recommended approach), GÖREV B lists 9 remaining cosmetic items (B1 `mesaj`/`saygi` tabs have zero `.surface`; B2 `SeyFx.transition` is dead; B3 M7 motion-token compliance 0.62; B5 the page weather layer is capped by the `FX2-23.10` opacity ceiling of 0.30 and raising it needs user approval). **Hardest constraint:** `tests/app/test_fx2_ambience.js` requires `amb-wx-*` selectors to target only `#sey-aurora::after` with opacity ≤ 0.30 — this is why the header uses its own `sky-time-*`/`sky-wx-*` namespace. Never mention `amb-wx-`/`amb-time-` inside a CSS comment: the contract scanner is a plain text scan and will misattribute the following block. `fx-coverage --gate` genuinely exits 1 (M7 below the 0.80 threshold, an approved ceiling) — do not read `$?` after a pipe. No push, no deploy; K3 device acceptance comes only from the user.
