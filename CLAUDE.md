...
- **İlham & İbadet Premium (IIP)** — [`ilham-ibadet-premium-plan/`](ilham-ibadet-premium-plan/README.md) — 24 kartlık İlham & İbadet hub yenileme programı. Durum kaynağı [`IIP-STATE.json`](ilham-ibadet-premium-plan/IIP-STATE.json); kanıt [`evidence/`](ilham-ibadet-premium-plan/evidence/); takip [`tracking/`](ilham-ibadet-premium-plan/tracking/); kart kataloğu [`06-UYGULAMA-KARTLARI.md`](ilham-ibadet-premium-plan/06-UYGULAMA-KARTLARI.md). **Durum (2026-09-22 denetimi):** state 24/24 done beyan ediyor; plan araçları (`plan-check.mjs`, `--self-test`, `.integration.py`) yeşil, ancak `deviceAcceptance=not_verified` ve hedef cihaz p95 ölçülmedi. Denetim tablosu [`evidence/DENETIM-BASELINE.md`](ilham-ibadet-premium-plan/evidence/DENETIM-BASELINE.md); sıralı kapatma adımları [`DUZELTME-PROMPTLARI.md`](ilham-ibadet-premium-plan/DUZELTME-PROMPTLARI.md) (P00→P17). Fixture'lar `tests/app/test_iip_*.js` (16 dosya). **KARDEŞ-KART REGRESYON KAPISI (P13):** `tests/FIXTURE-MAP.json` + `plan-check.mjs` — bir kart paylaşılan üretim dosyasına yazarsa o dosyanın **koruyucu fixture'larını** `fixtureFiles`'ta beyan etmek zorundadır; harita `node tools/fixture-map-build.mjs --write` ile yenilenir. IIP-20/21 bunu atladığı için IIP-05/06/09/12/13 gate'leri sessizce bozuldu (30 fixture saygi.js'e bağlı, kartlar 3'ünü beyan etmişti). **PERFORMANS:** `node tests/app/test_iip_perf_measure.js` yerel VM ölçer (p50 0.43–0.62 ms, p95 1.14–1.48 ms); **cihaz** p95 ≤200 ms hedefi ÖLÇÜLMEDİ — protokol [`evidence/IIP-PERF-PROTOKOL.md`](ilham-ibadet-premium-plan/evidence/IIP-PERF-PROTOKOL.md).
