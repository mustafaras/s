...
app/core/{prayer,zikir,quran,saygi,motivation,crisis,journal,health,
  library,report,map,profile,settings,reminders,messaging}.js
                 ...
                 `reminders.js` sits on top of the frozen REM×4 modules; app.js
                 keeps 22 `reminder*Legacy` inline bodies as the recorded
                 fail-safe fallback (MON-40) for when the registry is absent.
...
app/core/reminder*.js  Frozen REM program modules (`ReminderCatalogV1`,
                 ...
                 reminderCatalog.js.
app.js           The Şeyma runtime shell (single IIFE, ~13.1k lines after the
                 MON-01…60 split). Still the sole owner of `var data/ui/dark`
                 and the 9 `data=` rebind sites, the 7 B1 live getters, the
                 `SeyOnSyncState`/`SeyOnSynced` callbacks, all timer/listener
                 registrations, `window.App=App` and the 556 `App.x=function`
