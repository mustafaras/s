...
> - **2026-07-30:** **🕌 İman Köşesi — Detaylı namaz takibi (Diyanet vakitleri + konum)** (`mustafaras-iman-kosesi-plani` branch): Saygı sekmesinin içine yeni "İman Köşesi" premium kartı eklendi; her vakit (`Fecr`, `Sabah`, `Öğle`, `İkindi`, `Akşam`, `Yatsı`) için `performed`, `inCongregation`, `late`, `madeUp`, `nafile` ve `note` alanlarıyla detaylı takip yapılıyor. Vakitler `api.aladhan.com/v1/timings?method=13` (Diyanet hesabı) + `Europe/Istanbul` zaman dilimi kullanılarak çekiliyor; konum için GPS veya elle 81 il seçimi (`PRAYER_CITIES`) destekleniyor, son çekilen vakitler `localStorage` cache'inde 48 saat saklanarak offline fallback sağlıyor. Veri modeli: `data.days[date].prayer` (altı vakit) + `data.settings.prayer` (metot, konum, ayarlar) + `migrate()` backfill. Tam ekran overlay `openFaithCorner()`/`closeFaithCorner()` + `ui.faithOpen` deseniyle açılıyor; `render()`'daki `curOverlay`/`lastOverlay` mekanizmasına dahil. Panel aynası: "İman Köşesi" bento KPI kartı (kılınan/cemaat/kaza/nafile sayıları, son vakit durumu) + seçili gün detayındaki vakit satırı. `--faith` accent değişkenleri light/dark temalara tanımlandı; yeni `mosque` ve `users` ikonları `ICONS`'a eklendi. Cache bump `20260730b`. **Revizyon pass:** Saygı sekmesi içinde iki ayrı zengin preview kartı oluşturuldu: `saygiPreviewCardHTML()` (günün öncüsü thumbnail, alan/dönem, okundu/bekliyor rozet, dil chip, aç oku) ve `faithCornerCardHTML()` (şehir, 6 vakit pill bari, kılınan/cemaat/kaza/nafile rozetleri, aç oku); `saygiPreviewHubHTML()` bu kartları üst üste saran nefes alan bir hub olarak `saygiHTML()`'de intro ile makale arasına yerleştirildi. Alt navigasyondaki Saygi butonu yenilendi: label "Saygı·İman", `saygiPending` badge okunmamış makale veya tamamlanmamış namaz durumunda altın gradient rozet gösteriyor. Yeni CSS sınıfları: `.saygi-preview-hub`, `.sg-preview-card`, `.sg-preview-saygi`, `.sg-preview-faith`, `.sg-preview-prayerbar` ile vakit pill'leri (done/current/next). Headless testler (`prayer-harness.mjs`, `run-seyma/driver.mjs`) tamamı PASS. Henüz kullanıcı onayına sunulmadı.
