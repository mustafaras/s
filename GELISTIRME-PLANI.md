...
| 31 | 🌴 Tatil Modu — premium pause + su hedefi 10 bardak | 2 | ✅ | `data.settings.vacation` (`enabled`, `startAt`, `endAt`, `preset`, `reason`); Günışığı hava kartı üstünde ince premium tatil kartı; açıkken tarih inputları + preset/not; tatil günlerinde su hedefi `VACATION_WATER_GOAL=10`; `waterGoalCups(date)` ile tüm su gösterimleri dinamik; `currentStreak()` / `bestStreak()` tatil günlerini pause sayıp seriyi kırmaz; panelde Tatil Modu KPI kartı + gün-detay rozet + su/streak pause aynası; `setVacationEnabled`/`setVacationStart`/`setVacationEnd` su hedefi + hero stats + seri göstergesini anında senkronize eder; cache bump `20260723b` (2026-07-23, onay bekliyor) |
