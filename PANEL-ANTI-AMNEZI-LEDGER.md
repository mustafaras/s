...
| 3.1 | C2, C3 | `done` | a82296e | test_panel_event_focus 12/12, test_panel_p3_timeline_drawer 7/7, test_panel_p2_event_log 11/11, test_panel_p6_qa_release 16/16, test_panel_p5_responsive_a11y 24/24 PASS; grep openEventDrawerP\|closeEventDrawerP\|eventDetailsP\|statusBadgeP\|setEventDrawerLevelP panel.js → 0 sonuç; node --check panel.js OK | — | 2026-08-06 | openEventDrawerP/closeEventDrawerP/eventDetailsP/statusBadgeP/setEventDrawerLevelP silindi; UI.eventSelectedId/eventSelectedGroupKey/eventDrawerLevel ve EVENT_DRAWER_RETURN_ID temizlendi; cnt(rec) tekrarı (eski :979 civarı) silindi, tek tanım kaldı; eventDrawerKeydownP yalnız D4 modül drawer'ını yönetecek şekilde sadeleştirildi. D4 modül drawer'ı (openD4ModuleDrawerP/closeD4ModuleDrawerP/d4ModuleDrawerHTMLP) dokunulmadan test_panel_p3_timeline_drawer içinde regresyonsuz PASS. cache-bust panel.js→20260806c aynı commit'te yapıldı (madde 4b). test_panel_v2_* dosyalarındaki 3 hata bu değişiklikle ilgisiz — git stash ile doğrulandı, aynı hatalar değişiklik öncesi main'de de var (panel v2 kapsam dışı). Push+deploy tamamlandı, run 31080508692 success. |
| 3.2 | C1 | `done` | b4e55c2 | test_panel_event_focus 12/12, test_panel_p0_sync 31/31, test_panel_p1_projection 35/35, test_panel_p2_event_log 11/11, test_panel_p2_polling 15/15, test_panel_p2_sync 8/8, test_panel_p3_root_modules 35/35, test_panel_p3_timeline_drawer 7/7, test_panel_p4_provenance 28/28, test_panel_p5_responsive_a11y 24/24, test_panel_p6_qa_release 16/16 PASS (kabul kriterlerindeki TÜM dosyalar) + test_panel_focus_location, test_panel_p4_module_cards dahil geniş taramada regresyonsuz PASS; ayrıca 21 bilinen+1 bilinmeyen status kodu için p3StatusP/d2StatusBadgeP/statusToneForCodeP çıktısı eski implementasyonla byte-byte doğrulandı | — | 2026-08-06 | Kanonik panelStatusP(code)->{label,cls,tone} eklendi (p3StatusP'nin haritasından, en kapsamlı). p3StatusP, statusToneP, statusToneForCodeP, d2StatusBadgeP artık kendi mantıkları yok, panelStatusP/panelStatusBadgeHTMLP/panelLegacyBadgeHTMLP'yi çağıran ince wrapper'lar; syncRibbonHTMLP/coverageRibbonHTMLP/eventLogCardInnerHTMLP'nin üç ayrı localStatus() kapanışı da panelLegacyBadgeHTMLP'ye yönlendirildi. Tutarsızlık düzeltmesi: coverageRibbonHTMLP'nin localStatus'ü metin tabanlı 'bekliyor/...' çıkarımını YAPMIYORDU (sync/eventLog ile tutarsızdı) — hiçbir test eski davranışı sabitlemediği için diğer ikisiyle birleştirildi (panel.js içinde yorum satırıyla not edildi). auditRollupStatusP'nin rollup mantığı (öncelik: error>incomplete>ok>missing) dokunulmadı, ürettiği tek kod zaten p3StatusP()→panelStatusP() zincirinden geçiyor. Bu mantığı VM izolasyonunda extract eden ~10 test dosyasının extraction listesine yeni yardımcı fonksiyon adları (statusToneP/panelToneOverrideP/panelStatusP/panelStatusBadgeHTMLP/panelLegacyBadgeHTMLP) eklendi. cache-bust panel.js→20260806d aynı commit'te yapıldı (madde 4b). Push+deploy tamamlandı, run 31082940969 success. |
| 4.1 | C4 | `done` | 1a92ac6 | test_faz11_panel.js 50/50 PASS (yeni profileAssessmentDataP mock eklendi) + 16 diğer panel/faz10 test dosyası regresyonsuz PASS; ayrıca psychCardHTML 6/6, profileAssessmentCardHTML 10/10, quranJourneyPanelCardHTML 6/6 sentetik fixture node:vm'de eski implementasyonla byte-byte doğrulandı (panel.js düzenlemesinden ÖNCE ve SONRA iki kez) | — | 2026-08-06 | psychCardHTML/profileAssessmentCardHTML/quranJourneyPanelCardHTML her biri *DataP() (saf hesap, HTML yok) + aynı imzalı render fonksiyonu olarak ikiye ayrıldı; render() fonksiyonuna dokunulmadı (kapsam dışı). cache-bust panel.js→20260806e aynı commit'te yapıldı (madde 4b). Push+deploy bekliyor. |
| 4.2 | C5 | `done` | 241767b | 17 kapsam-içi panel/faz10/faz11 test dosyası regresyonsuz PASS (test_panel_p0_sync 31/31, p3_root_modules 35/35, p4_provenance 28/28, p4_module_cards 13/13, focus_location 13/13 dahil — hepsi PROJECTION.* mock'larına güncellendi); grep ile panel.js+tests/ içinde eski dört global isminin (OBSERVER_PROJECTION_PATH hariç) hiçbir çağrı sitesinde kalmadığı doğrulandı | — | 2026-08-06 | OBSERVER_PROJECTION/PROJECTION_STATE/PROJECTION_SECTIONS/SECTION_FETCH_STATE tek PROJECTION={snapshot,state,sections,sectionFetchState} objesinde birleştirildi (mekanik, kelime-sınırlı rename + panelSig() tek JSON.stringify(PROJECTION)'a indirgendi). Bulunan regresyon: rename sonrası 3 `typeof PROJECTION.x!=='undefined'` guard'ı artık VM test context'lerinde ReferenceError-güvenli değildi (undeclared PROJECTION üzerinde .prop erişimi typeof içinde bile fırlar) — `typeof PROJECTION!=='undefined'&&PROJECTION&&...` eklenerek düzeltildi. "hadSections" side-channel hata-koruma dalı davranışsal olarak dokunulmadı. cache-bust panel.js→20260806f aynı commit'te yapıldı (madde 4b). Push+deploy bekliyor. |
