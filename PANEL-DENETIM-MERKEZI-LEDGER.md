...
| D3.1 | §4, §9 Faz 3 | `done` | 23b3375 | node --check OK, grep auditEntryHTMLP()/eventLogCardHTMLP() render() içinde 0 sonuç | — | 2026-08-06 | render():h+=auditEntryHTMLP() ve h+=eventLogCardHTMLP() satırları silindi; fonksiyon tanımları/auditPaneHTMLP kullanımı korundu |
| D3.2 | §8 | `done` | 23b3375 (D3.1 ile aynı commit) | test_panel_focus_location.js 13/13 PASS (1 assertion güncellendi) + 27 diğer panel test dosyası regresyonsuz PASS (3 bilinen v2 çökmesi kapsam dışı) | — | 2026-08-06 | **Plan §8'deki 8 dosyalık liste bir hipotezdi — gerçek FAIL sadece 1 dosyaydı** (test_panel_focus_location.js, "normal dashboard hedef kartlarını tekrar basmaz" assertion'ı auditEntryHTMLP()'nin artık render()'da olmamasını da kapsayacak şekilde güncellendi). Diğer 7 aday dosya etkilenmedi, bu yüzden D3.1 ile aynı commit'te birlikte tamamlandı. **Faz 3 tamamlandı.** |
| D4.1 | §4, §9 Faz 4 | `done` | (kod değişikliği yok) | 84 `.audit-*`/`.p3-*`/`.p4-*`/`.d4-*`/`.event-log-*` sınıfı tek tek grep ile denetlendi; 27 panel test dosyası zaten regresyonsuz PASS (değişiklik yapılmadığı için etkilenmedi) | — | 2026-08-06 | **Beklenenin aksine silinecek HİÇBİR sınıf bulunamadı.** D3.1 yalnızca render()'daki ÇAĞRILARI kaldırdı, fonksiyon TANIMLARI (dev-mode'un kullandığı rootModulesCardHTMLP/p4ProvenanceCardHTMLP/d4ModuleAtlasHTMLP/auditEntryHTMLP/eventLogCardHTMLP) hâlâ yaşıyor ve bu sınıfları üretiyor; D1.2'nin curatedChangeLogCardHTMLP()'si de `.event-log-*` sınıflarının çoğunu birincil yüzeyde yeniden kullanıyor. İlk grep taramasında "0 kullanım" görünen 12 sınıf (d4-coverage-*, d4-module-status-*) yanlış pozitifti — bunlar `'d4-coverage-'+m` gibi dinamik string birleştirmeyle üretiliyor, literal grep bunu yakalamıyor; panel.js:1489/1536'da doğrulandı. Prompt'un "emin olmadığın sınıfı silme" kuralı gereği HİÇBİR SİLME yapılmadı. **Faz 4 tamamlandı (no-op).** |
| D5.1 | §9 Faz 5 | `done` | da3faec (Faz 4 sonu) | 28/28 tests/test_panel_*.js dosyası taranadı — 25 panel v1 test dosyası regresyonsuz PASS, 3 bilinen test_panel_v2_* çökmesi (kapsam dışı, D3.1'den önce de mevcuttu) hariç; node --check panel.js OK; panel.html script-tag balance 8/8 | (aşağıda) | 2026-08-06 | cache-bust doğrulandı: panel.js?v=20260806n (D3.1+D3.2 commit'i ile güncel), panel.css/panelCoverageManifest.js bu girişimde değişmedi (D4.1 no-op), bump gerekmedi. Ledger'daki D1.1-D4.1 satırlarının hepsi `done`. **Tüm fazlar (1-5) tamamlandı.** |
