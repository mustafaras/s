...
| 18 | Genel Bakış Sayfası Yenileme | ✅ TAMAMLANDI | `0ba4c4a` (önceki kod: `edcf69d`) | `node --check panel-v2.js`; `node --check panelCoverageManifest.js`; `node tests/test_panel_v2_today.js`; `node tests/test_panel_v2_day_detail.js`; `tests/test_panel_v2_*.js` (11/11 PASS); `git diff --check`; secret scan PASS; Pages run `31367317356` SUCCESS | Kullanıcı düzeltmesi: konum geçmişi kapalı expander, zikir/Esmâ KPI + preset dökümü + tefekkür ayrıntıları, premium header/navigasyon ayrımı, senkron düğmesi stili ve yatay tarih seçiciler; cache bump `2026081019` |
