...
| 25 | Swipe Gesture | ✅ TAMAMLANDI | `bdd9027` | `node --check panel-v2.js`; `node --check panelCoverageManifest.js`; `node tests/test_panel_v2_swipe.js`; `tests/test_panel_v2_*.js` (12/12 PASS); `git diff --check`; secret scan PASS; Pages run `31397912918` SUCCESS; canlı cache smoke PASS | Mobil-only `touchstart`/`touchmove`/`touchend`/`touchcancel`, 50px yatay eşik, sola sonraki/sağa önceki gün, dikey scroll ve interaktif hedef çakışma önleme, listener cleanup, `matchMedia` viewport gate, `touch-action: pan-y`, cache bump `2026081028`; manuel browser testi data-safety lock nedeniyle çalıştırılmadı, eşdeğer headless touch fixture PASS |
