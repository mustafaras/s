...
function ayarlarHTML(){
  ...
  var kaoHidden=!!(data.quranLearn&&data.quranLearn.settings&&data.quranLearn.settings.kaoVisible===false);
  if(sgh.hideLocationCard||sgh.hideRepoBanner||sgh.hideVacationCard||kaoHidden){
    ...
    if(kaoHidden) h+='<button onclick="App.kaoToggleVisible()" style="min-height:44px;display:flex;align-items:center;gap:9px;border:1px solid var(--field-bd);cursor:pointer;padding:12px 13px;border-radius:13px;font-size:var(--f-subhead);font-weight:700;color:var(--text);background:var(--card);"><span style="display:inline-flex;color:var(--quran-mid);">'+icon('book-open',16)+'</span><span style="flex:1;text-align:left;">Kur’an Arapçası kartını geri getir</span></button>';
    h+='</div>';
  }
