...
**Paket:** A · **Rol:** domain-review · **Boyut:** M · **Durum:** yalnız [IIP-STATE.json](../IIP-STATE.json) üzerinden okunur.
...
Görev metninin tek kaynağı: [UYGULAMA-PROMPTLARI.md](../UYGULAMA-PROMPTLARI.md), **IIP-03** bölümü. Normal oturumda `session-brief.mjs IIP-03` bu bölümü ve aşağıdaki canlı sözleşmeyi birlikte verir; kartın tamamını ayrıca okumak gerekmez.

<!-- CONTRACT:START -->
## Yürütme sözleşmesi — üretilmiş

**Bağımlılıklar:** IIP-01.

**Rol:** domain-review · **Kararlar:** DEC-03.

**Üretim allowlist:** `app/core/saygi.js`, `panel/panel.js`.

**Test allowlist:** `tests/app/test_saygi_boundary.js`, `tests/app/test_prayer_boundary.js`, `tests/app/test_iip_03.js`.

**Gerekli gate:** scope, requirements, review, source. **Veri etkili yazıcı:** false.

Plan artifactleri: `evidence/IIP-03/`. State/ledger tek yazarı integratör. Allowlist dışı üretim değişimi ve canlı veri yazımı yok. Yeni dosya ihtiyacı kapsam kaydıyla çözülür; var olan yetki tekrar sorulmaz.

### REQ-005 / TC-005 — Zaman ve kayıt anlamı

fajr/sunrise dahil altı alanın tarihsel UI, API ve panel anlamı belgelenir.

Olumsuz kontrol: Sunrise eski kaydı belirsizse beş vakte otomatik dönüştürülmez.

### REQ-006 / TC-006 — Payda belirsizliği

Nesne varlığı gerçek kayıt sayılmaz; bilinmeyen gün, sıfır ve zaman kapsamı ayrı tanımlanır.

Olumsuz kontrol: Migration ile oluşmuş boş prayer nesnesi bilinçli sıfır diye raporlanmaz.
<!-- CONTRACT:END -->
