...
| AD-28 | Kaydırma kenarı efekti | ✅ TAMAMLANDI | `PENDING` | S4+S5+yapı geçti | `.sey-appheader` blur'ü `--hdr-blur` değişkenine alındı; **dinlenme değeri 26px'te bırakıldı** (istenmeyen görsel değişiklik olmasın), `.is-scrolled` durumunda 34px'e çıkıyor. `backdrop-filter` mevcut transition listesine eklendi. Yeni JS yok — `is-scrolled` sınıfını yazan `app.js:16051` dokunulmadı (I4). **Yan bulgu:** header'ın kendi `transition`'ı hiçbir `prefers-reduced-motion` bloğunda kapatılmıyordu (mevcut boşluk); blur'ü transition'a eklemek boşluğu büyütecekti — header için reduced-motion kuralı eklendi. **Gözlem (uygulanmadı):** koyu temada header hâlâ `backdrop-filter:none` + opak `#050506`. Opak navigasyon çubuğu meşru bir iOS kalıbı olduğu için AD-26 kapsamı dışında bırakıldı; istenirse ayrı bir karar. |
