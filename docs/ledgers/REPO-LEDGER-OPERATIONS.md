...
| REPO-M004 | 2026-08-03 | M4 test fixture organization | Root’taki 21 sentetik Node test fixture’ı `tests/` klasöründe toplandı; ortak repo-root resolver ve güncel komut yolları eklendi | `tests/`, `tests/repo-root.js`, `tests/README.md`, AGENTS/CLAUDE/skill ve güncel doküman referansları | Core/test syntax **27/27**; root’tan **21/21**; `tests/` cwd’den **21/21**; app driver, Zikirmatik **90/90**, B1, B2 **32/32**, B3 **20/20**, diff check PASS; Pages validate/deploy success | Runtime/app/sync/panel script sırası, `data/`, localStorage, `seyma-data`, sync ve observer write kanalları değişmedi; yalnız GitHub repo/Pages test yolları değişti |
