...
Görev metninin tek kaynağı: [UYGULAMA-PROMPTLARI.md](../UYGULAMA-PROMPTLARI.md), **IIP-06** bölümü. Normal oturumda `session-brief.mjs IIP-06` bu bölümü ve aşağıdaki canlı sözleşmeyi birlikte verir; kartın tamamını ayrıca okumak gerekmez.
...
**Test allowlist:** `tests/app/test_saygi_boundary.js`, `tests/app/test_prayer_boundary.js`, `tests/app/test_iip_06.js`.

**Gerekli gate:** scope, requirements, review, source, visual. **Veri etkili yazıcı:** false.

Plan artifactleri: `evidence/IIP-06/`. State/ledger tek yazarı integratör. Allowlist dışı üretim değişimi ve canlı veri yazımı yok. Yeni dosya ihtiyacı kapsam kaydıyla çözülür; var olan yetki tekrar sorulmaz.

### REQ-011 / TC-011 — Vakit kayıt sunumu

Saat, kaynak, kayıt durumu, ayrıntı birbirine karışmaz; altı eski alan erişilir.

Olumsuz kontrol: Büyük yazıda saat ve kayıt kontrolü üst üste binmez.

### REQ-012 / TC-012 — Kıble güveni

Hesaplanan yön, cihaz yönü, hassasiyet ve sensör reddi ayrı okunur.

Olumsuz kontrol: Sensör yokken sahte canlı ibre gösterilmez; var olan hesap korunur.
<!-- CONTRACT:END -->
