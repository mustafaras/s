...
**Paket:** A · **Rol:** frontend · **Boyut:** M · **Durum:** yalnız [IIP-STATE.json](../IIP-STATE.json) üzerinden okunur.
...
Görev metninin tek kaynağı: [UYGULAMA-PROMPTLARI.md](../UYGULAMA-PROMPTLARI.md), **IIP-07** bölümü. Normal oturumda `session-brief.mjs IIP-07` bu bölümü ve aşağıdaki canlı sözleşmeyi birlikte verir; kartın tamamını ayrıca okumak gerekmez.
...
**Üretim allowlist:** `app/core/zikir.js`, `app/core/render.js`, `app/styles.css`.

**Test allowlist:** `tests/app/test_saygi_boundary.js`, `tests/app/test_zikir_boundary.js`, `tests/app/test_quran_boundary.js`, `tests/app/test_iip_07.js`.

**Gerekli gate:** scope, requirements, review, source, visual. **Veri etkili yazıcı:** false.

Plan artifactleri: `evidence/IIP-07/`. State/ledger tek yazarı integratör. Allowlist dışı üretim değişimi ve canlı veri yazımı yok. Yeni dosya ihtiyacı kapsam kaydıyla çözülür; var olan yetki tekrar sorulmaz.

### REQ-013 / TC-013 — Zikir kesintisizliği

Mevcut sayaç, hatim, manuel giriş, geri alma ve notlar aynı davranışla korunur.

Olumsuz kontrol: 100 hızlı giriş, arka plan render ve hatim sınırı testi kayıpsız geçer.

### REQ-014 / TC-014 — Kur’an kesintisizliği

Sûre durumu, video ve notlar görsel iyileştirme sırasında korunur.

Olumsuz kontrol: Not caret ve video düğümü arka plan güncellemesinde yeniden kurulmaz.
<!-- CONTRACT:END -->
