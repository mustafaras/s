...
Görev metninin tek kaynağı: [UYGULAMA-PROMPTLARI.md](../UYGULAMA-PROMPTLARI.md), **IIP-15** bölümü. Normal oturumda `session-brief.mjs IIP-15` bu bölümü ve aşağıdaki canlı sözleşmeyi birlikte verir; kartın tamamını ayrıca okumak gerekmez.

<!-- CONTRACT:START -->
## Yürütme sözleşmesi — üretilmiş

**Bağımlılıklar:** IIP-14.

**Rol:** domain · **Kararlar:** DEC-03.

**Üretim allowlist:** `app/core/saygi.js`, `panel/panel.js`, `panel/v2/panel-v2.js`.

**Test allowlist:** `tests/app/test_saygi_boundary.js`, `tests/app/test_prayer_boundary.js`, `tests/app/test_iip_15.js`.

**Gerekli gate:** scope, requirements, review, source, visual. **Veri etkili yazıcı:** false.

Plan artifactleri: `evidence/IIP-15/`. State/ledger tek yazarı integratör. Allowlist dışı üretim değişimi ve canlı veri yazımı yok. Yeni dosya ihtiyacı kapsam kaydıyla çözülür; var olan yetki tekrar sorulmaz.

### REQ-029 / TC-029 — Dürüst rapor

Vakit/zikir/okuma ayrı toplam; anlamsız birleşik skor veya belirsiz yüzde yok.

Olumsuz kontrol: Gün yok, yalnız not, artık yıl ve eski kayıt örneği bağımsız hesapla aynı.

### REQ-030 / TC-030 — Panel eşliği

Current-panel ile uygulama aynı kayıt tanımını kullanır; Panel-v2 ayrı regresyon.

Olumsuz kontrol: Eski veri ve karma kayıtlar iki yüzeyde anlam değiştirmez.
<!-- CONTRACT:END -->
