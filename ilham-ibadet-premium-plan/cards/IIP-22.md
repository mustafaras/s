...
**Paket:** C · **Rol:** platform · **Boyut:** L · **Durum:** yalnız [IIP-STATE.json](../IIP-STATE.json) üzerinden okunur.
...
Görev metninin tek kaynağı: [UYGULAMA-PROMPTLARI.md](../UYGULAMA-PROMPTLARI.md), **IIP-22** bölümü. Normal oturumda `session-brief.mjs IIP-22` bu bölümü ve aşağıdaki canlı sözleşmeyi birlikte verir; kartın tamamını ayrıca okumak gerekmez.
...
**Rol:** platform · **Kararlar:** DEC-06.

**Üretim allowlist:** `sw.js`, `index.html`.

**Test allowlist:** `tests/app/test_saygi_boundary.js`, `tests/app/test_iip_22.js`.

**Gerekli gate:** scope, requirements, review, source, offline. **Veri etkili yazıcı:** false.

Plan artifactleri: `evidence/IIP-22/`. State/ledger tek yazarı integratör. Allowlist dışı üretim değişimi ve canlı veri yazımı yok. Yeni dosya ihtiyacı kapsam kaydıyla çözülür; var olan yetki tekrar sorulmaz.

### REQ-043 / TC-043 — Genel offline paket

Yalnız onaylı genel içerik; boyut/sürüm/kota/kaldırma ve kurulum durumu açık.

Olumsuz kontrol: Tokenlı cevap veya kişisel JSON cache manifestine giremez.

### REQ-044 / TC-044 — Güvenli güncelleme

Aktif sayaç/not sırasında yeni SW sürümü zorla reload yapmaz.

Olumsuz kontrol: Kısmi paket, kota hatası ve eski manifestten dönüş kullanımı kilitlemez.
<!-- CONTRACT:END -->
