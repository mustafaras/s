...
**Paket:** C · **Rol:** reviewer · **Boyut:** L · **Durum:** yalnız [IIP-STATE.json](../IIP-STATE.json) üzerinden okunur.
...
Görev metninin tek kaynağı: [UYGULAMA-PROMPTLARI.md](../UYGULAMA-PROMPTLARI.md), **IIP-23** bölümü. Normal oturumda `session-brief.mjs IIP-23` bu bölümü ve aşağıdaki canlı sözleşmeyi birlikte verir; kartın tamamını ayrıca okumak gerekmez.

<!-- CONTRACT:START -->
## Yürütme sözleşmesi — üretilmiş

**Bağımlılıklar:** IIP-20, IIP-21, IIP-22.
...
Plan artifactleri: `evidence/IIP-23/`. State/ledger tek yazarı integratör. Allowlist dışı üretim değişimi ve canlı veri yazımı yok. Yeni dosya ihtiyacı kapsam kaydıyla çözülür; var olan yetki tekrar sorulmaz.

### REQ-045 / TC-045 — Tam zincir kabul

App/current-panel/Panel-v2, state/sync ve görsel kanıt ayrı raporlu.

Olumsuz kontrol: Önemli bir fonksiyonun yalnız markup varlığıyla geçtiği kabul edilmez.

### REQ-046 / TC-046 — Gerçek cihaz sınırı

iPhone Safari/PWA, Android Chrome ve klavye/ekran okuyucu örnekleri kayıtlı.

Olumsuz kontrol: Ajan VM çıktısı kullanıcı cihazı sonucu diye doldurulmaz.
<!-- CONTRACT:END -->
