...
function quranBadgeClassP(tone){ return tone==='wait'?'b-warn':tone==='ready'?'b-quranp':tone==='done'?'b-ok':tone==='warn'?'b-danger':'b-dim'; }
...
function quranJourneyPanelCardHTML(){
  ...
  if(!rows.length){
    ...
    rows.forEach(function(r){
      ...
      det+='<span class="badge '+quranBadgeClassP(rs.tone)+'">'+esc(rs.label)+'</span>';
      det+='</div>';
...
function p4ProvenanceCardHTMLP(){
  ...
  var tc=th.consent||{}, dc=th.decision||{}, sh=th.share||{}, wd=th.windDown||{}, counts=nt.counts||{};
  ...
  h+='<div class="p3-kv"><span>Observer kabul</span><b>'+p3TimeP(nt.observerReceipt&&nt.observerReceipt.acceptedAt)+'</b></div></div>';
...
function pmRow(side,tone,text,footHtml,enter){
  var cls=side==="out"?("out-"+(tone||"aeon")):"in";
  var h='<div class="pm-row '+side+(enter?" pm-enter":"")+'">';
  h+='<div class="pm-bubble '+cls+'">'+pmClamp(text)+'</div>';
  if(footHtml) h+='<div class="pm-foot">'+footHtml+'</div>';
  h+='</div>';
  return h;
}
...
function pmRowMedia(side,tone,mediaKind,mediaId,w,h,footHtml,enter){
  var cls=side==="out"?("out-"+(tone||"aeon")):"in";
  var h2='<div class="pm-row '+side+(enter?" pm-enter":"")+'">';
...
function aeonThreadCardHTML(){
  ...
  s+='</div>';
  s+='<div class="pm-thread" id="pm-aeon-thread" style="max-height:360px;">';
  if(!items.length){
    ...
    items.forEach(function(it){
      ...
      if(ds && ds!==prev){ s+='<div class="pm-daydiv">'+esc(pmDay(it.time))+'</div>'; prev=ds; }
      if(it.side==="in"){
        ...
        s+=it.mediaKind?pmRowMedia("in",null,it.mediaKind,it.mediaId,it.w,it.h,f,false):pmRow("in",null,it.text,f,false);
      } else {
        ...
        s+=it.mediaKind?pmRowMedia("out","aeon",it.mediaKind,it.mediaId,it.w,it.h,f2,false):pmRow("out","aeon",it.text,f2,false);
      }
    });
  }
  s+='</div>';
  ...
  if(UI.aeonRecActiveP){
    ...
    s+='<textarea id="pm-aeon-input" oninput="aeonChatDraft(this.value)" onkeydown="pmAeonKeydown(event)" placeholder="ÆON olarak yaz…" rows="1">'+esc(UI.msgDraft||"")+'</textarea>';
...
function coreStripHTML(){
  ...
  var s='<div class="card pad" style="margin-bottom:12px;display:flex;align-items:center;gap:14px;flex-wrap:wrap;border-color:var(--bd-gold);background:linear-gradient(135deg,rgba(212,175,55,.07),rgba(28,28,40,.74));">';
...
function weatherCardHTML(){
  var loc=(D&&D.location&&typeof D.location.lat==="number"&&typeof D.location.lng==="number")?D.location:null;
...
function render(){
  ...
  h+='<div class="topbar">';
  h+='<div class="brand">';
  h+='<span class="brand-mark" style="color:#1a1404;font-weight:800;">⬡</span>';
  h+='<span class="brand-name">ÆON</span>';
  h+='</div>';
  h+='<div class="tb-chips">';
  if(DEMO_MODE) h+='<span class="badge b-warn">DEMO · ağ kapalı</span>';
  h+='<span class="badge '+freshClass+(fresh.klass==="ok"?" pulse":"")+'">'+fresh.txt+'</span>';
  h+='<span class="badge '+rskClass+'">'+rsk.txt+'</span>';
  h+='<span class="'+locBadgeClass+(loc&&locAgeMin<120?" pulse":"")+'" style="display:inline-flex;align-items:center;gap:4px;">'+icon('map-pin',12)+' '+locBadgeTxt+'</span>';
  h+='</div>';
  h+='<div class="tb-meta">';
  h+='<div class="meta"><b class="mono">'+(opened?new Date(opened).toLocaleString("tr-TR",{hour:"2-digit",minute:"2-digit",day:"2-digit",month:"2-digit",timeZone:"Europe/Istanbul"}):"—")+'</b><span>Son açılış</span></div>';
  ...
  h+='</div>';
  h+='<span style="font-size:var(--f2);color:var(--t4);font-weight:700;letter-spacing:.4px;text-transform:uppercase;">'+UI.range+' günlük pencere</span>';
  h+='<input type="date" value="'+esc(selected)+'" onchange="setSelectedDate(this.value)" style="margin-left:auto;">';
  h+='</div>';

  // ── JUMP-NAV: 5 bölüme atlama şeridi (scroll-spy ile aktif bölüm vurgulanır) ──
  h+='<div class="jumpnav" id="jumpnav">';
  ...
  // ROW 1: 6 KPI hero tiles (span-2 each)
...
function initLocMap(){
  ...
  var mvTrk=(D&&D.days&&D.days[today()]&&D.days[today()].movement&&Array.isArray(D.days[today()].movement.track))?D.days[today()].movement.track:[];
...
window.aeonChatDraft=function(v){
  UI.msgDraft=v;
...
window.aeonOpenImageP=function(mediaId){
  ...
  d.style.cssText="position:fixed;inset:0;z-index:900;background:rgba(3,3,5,0.94);display:flex;align-items:center;justify-content:center;padding:20px;";
  d.innerHTML='<img src="'+uri+'" style="max-width:100%;max-height:100%;border-radius:12px;box-shadow:0 20px 60px rgba(0,0,0,0.6);">'
    +'<button aria-label="Kapat" style="position:absolute;top:16px;right:16px;border:none;background:rgba(255,255,255,0.12);color:#fff;width:38px;height:38px;border-radius:50%;cursor:pointer;display:flex;align-items:center;justify-content:center;">'+icon('x',16)+'</button>';
