...
2. `tests/app/test_modularization_boundary.js` güncelle:
   ...
   - `app.js`’in hâlâ yüklü olduğunu doğrula. **Kritik (B1 kararı):** Faz -1.1'de `data`/`ui`/`dark`/`migrate`/`save` henüz `window`'da DEĞİLDİR (canlı getter'lar Faz 0 / FX-P-05'te eklenir). Bu yüzden `window.data`/`window.ui` varlığını assert ETME. Bunun yerine `window.App` varlığını ve `window.SeymaState`/`window.SeymaSave` getter'larının tanımlı olduğunu doğrula.
