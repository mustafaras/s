...
// IIP-10/11 sonrası KAO-10…16 yüzeyi 14 handler ekledi; 558 → 572. KAO-17 E7 ayarları 8 handler: 572 → 580. KAO-26 stüdyo 2 handler: 580 → 582. KAO-27 gölgeleme 5 handler: 582 → 587. KAO-28 âyet 2 handler: 587 → 589.
ok('App function assignment count remains 589', currentAssignments.length === 589);
const handlerSurface = new Set((appSource.match(/App\.[A-Za-z0-9_]+\s*=[^=]/g) || [])
  .map((value) => value.match(/App\.[A-Za-z0-9_]+/)[0]));
ok('unique App handler surface remains 751', handlerSurface.size === 751);
