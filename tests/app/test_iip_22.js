...
(async function () {
  ...
  await test('tokenlı, kişisel, panel, video ve dış URL cache dışıdır', () => {
    ...
    assert.equal(key(new SyntheticRequest('https://example.test/s/app.js?v=20260926c')), 'https://example.test/s/app.js?v=20260926c');
  });
  ...
  await test('eski sürüme dönüş exact allowlist isteğini kullanıma açık tutar', async () => {
    const runtime = loadSw();
    const old = await runtime.caches.open('seyma-offline-v1-iip22-old');
    const url = 'https://example.test/s/app.js?v=20260926c';
  ...
  await test('kalıcı yüzen Offline paneli kaldırılır; güvenli SW kaydı korunur', () => {
    ...
    assert.match(indexSource, /navigator\.serviceWorker\.register\('sw\.js\?v=20260926c'\)/);
  ...
  await test('aktif sayaç/not durumu SW güncellemesinden bağımsızdır', () => {
    const registrationBlock = indexSource.slice(indexSource.indexOf("navigator.serviceWorker.register('sw.js?v=20260926c')"));
