...
function frozenRows(verified, examplePronunciations) {
  ...
  return verified.lemmas.map((record) => {
    ...
    assert(Array.isArray(record.examples) && record.examples.length > 0,
      `${record.lemmaId}: örnek gerekli`);
    return [
      record.lemmaId,
      record.ar,
      record.translit.tr,
      [record.tr1, record.tr2].filter(Boolean),
      record.root,
      record.pattern,
      record.pos,
      record.freq,
      ...
      record.examplesException ? {
        ...
        final: record.examplesException.final === true,
        rootOccurrences: record.examplesException.rootOccurrences,
...
function packFrozenRows(rows) {
  const raw = JSON.stringify(rows);
  const counts = new Map();
  for (const match of raw.matchAll(/[\p{L}\p{M}ٱ]+/gu)) {
    const word = match[0];
    if (Buffer.byteLength(word) < 5) continue;
    counts.set(word, (counts.get(word) || 0) + 1);
  }
  const dictionary = [...counts.entries()]
    .map(([word, count]) => ({
      word,
      gain: count * (Buffer.byteLength(word) - 3) - Buffer.byteLength(JSON.stringify(word)) - 1
    }))
    .filter((entry) => entry.gain > 0)
    .sort((a, b) => b.gain - a.gain || a.word.localeCompare(b.word))
    .slice(0, 512)
    .map((entry) => entry.word);
  let packed = raw;
  dictionary.forEach((word, index) => {
    packed = packed.split(word).join(String.fromCodePoint(0xE000 + index));
  });
...
function renderFrozenLexicon(verified, examplePronunciations) {
  ...
  // QuranLexiconV1 — pinned QAC/Tanzil corpus inputs and the D-12 verified
  // Turkish review layer. Generated by kao-lexicon-build --freeze; do not edit.
  var METHODOLOGY_TR = '524 lemma; QAC sıklık ve kök çözümlemesi ile Tanzil Uthmani örneklerinden derlendi. Türkçe anlam, kalıp ve örnek çevirileri D-12 doğrulama akışından geçti. Seyrek köklerde yalnız belgeli D-13 nihai istisnası üç örnek eşiğinin altında kalabilir.';
  var ATTRIBUTION = Object.freeze({
    generatedAt: ${JSON.stringify(verified.importedAt)},
    verification: ${JSON.stringify(verified.approvalRule)},
    sources: Object.freeze([
      Object.freeze({ name:'Quranic Arabic Corpus morphology 0.4', license:'GNU GPL', url:'https://corpus.quran.com/download/' }),
      Object.freeze({ name:'Tanzil Uthmani text', license:'CC BY 3.0', url:'https://tanzil.net/download/' }),
      Object.freeze({ name:'KAO Turkish review layer', license:'project-authored', note:'D-12 verified; reference translations are not copied into this package' })
    ])
  });
  ...
  lemmas.forEach(function(lemma){
    index[lemma.id]=lemma;
    if(lemma.root){
      if(!roots[lemma.root]) roots[lemma.root]=[];
      roots[lemma.root].push(lemma.id);
    }
  });
  Object.keys(roots).forEach(function(root){ roots[root]=Object.freeze(roots[root]); });
  window.QuranLexiconV1=Object.freeze({
    version:${JSON.stringify(verified.version)},
    METHODOLOGY_TR:METHODOLOGY_TR,
...
function freezeLexicon() {
  ...
  console.log(`lemmas=${verified.lemmas.length} bytes=${size}`);
}
...
async function main(argv) {
  ...
  if (argv.includes('--freeze')) {
    rejectUnknown(argv, new Set(['--freeze']));
    freezeLexicon();
    return;
  }
